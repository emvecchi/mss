...
def write_to_files(images_pat, output_path):
    
    os.mkdir(output_path + '/sample_1')
    os.mkdir(output_path + '/sample_1/images')
    os.mkdir(output_path + '/sample_1/tags')
    os.mkdir(output_path + '/sample_051')
    os.mkdir(output_path + '/sample_051/images')
    os.mkdir(output_path + '/sample_051/tags')
    os.mkdir(output_path + '/sample_033')
    os.mkdir(output_path + '/sample_033/images')
    os.mkdir(output_path + '/sample_033/tags')
	...
	new_tag_file.write(sample_1[image])
	new_tag_file.close()
    for image in sample_051:
	old_image_path = images_path + '/image_name'
	new_image_path = output_path + '/sample_051/images/' + image_name
	new_tag_path   = output_path + '/sample_051/tags/' + image_name[:-4] + '.txt')
        shutil.copy(old_image_path, new_image_path))
	new_tag_file   = open(new_tag_path, 'w')
	new_tag_file.write(sample_1[image])
	new_tag_file.close()
	...
	old_image_path = images_path + '/image_name'
	new_image_path = output_path + '/sample_033/images/' + image_name
	new_tag_path   = output_path + '/sample_033/tags/' + image_name[:-4] + '.txt')
        shutil.copy(old_image_path, new_image_path))
	new_tag_file   = open(new_tag_path, 'w')
	new_tag_file.write(sample_1[image])
	new_tag_file.close()
